...
- 📘 **数据库设计**：`docs/database/数据库架构.md` - 官方数据库设计规范
- 📗 **技术决策**：`docs/progress/TECHNICAL_DECISIONS.md` - 关键技术决策记录
- 📙 **开发规范**：`docs/development/编码规范.md` - 编码规范（必读）
- 📕 **API 设计**：`docs/design/API设计.md` - RESTful API 规范
...
## 2026-09-05 - 优化计划第一、二批次实施

### 当前阶段

- ✅ 已完成仓库内安全止血、核心业务闭环修复、正式迁移脚本、基础 CI 和单一 Docker 运行拓扑的代码实现。
- ⚠️ Git 历史改写、外部凭据轮换、真实数据迁移和生产发布未执行，需独立授权和环境验收。
- ⚠️ 邮件持久化补漏/重试、爬虫租约锁与持久化调度、文件对象存储、完整集成/E2E 和 70% 核心覆盖率仍属后续批次。

### 已完成功能

- 安全：移除当前树的数据库备份、7 个导入 Excel 和真实生产环境文件；增加忽略规则、Gitleaks CI 和敏感资产处置记录。
- 敏感数据：实现 AES-256-GCM 带版本密文封装、HMAC 盲索引和旧明文读取兼容；增加默认只读的存量数据迁移脚本。
- 认证授权：取消随机 JWT fallback 和通用弱口令；增加首次改密标记、IP/账号双维限流、管理员/操作员/只读角色及路由权限中间件。
- API 和脱敏：Apple ID 列表不再返回密码和密保；取机人、订单和导出默认脱敏；订单/Excel 导出防表格公式注入。
- 业务闭环：修复 Apple ID/取机人状态口径、真实新增 API、取机人编辑字段错配、订单筛选选项、精确批量刷新、模板下载和订单导出。
- 导入：改为服务端一次性会话令牌，实施 15 分钟过期、用户归属、1000 行上限、自动事务收口和临时文件 `finally` 清理。
- 数据库：新增 `payment_screenshot` TEXT → JSONB、`orders.tag` 长度修复、敏感列扩容、取机人盲索引和角色迁移的可回滚 Migration。
- 部署：确定 Docker 单一拓扑，拆分 API、邮件 Worker 和爬虫 Worker；统一 3000 端口；增加 liveness/readiness、request ID 和异常退出。
- 工程：修正 ESLint 范围、文档一致性检查和 Pre-commit 中文路径；补充根目录和前端 lockfile；建立后端/前端/Secret Scan CI 基线。

### 关键实现笔记

- 存量敏感数据不在应用启动时自动改写；`npm run security:migrate` 仅盘点，显式加 `--execute` 才在事务中迁移。
- API 文档确立 `camelCase` 目标契约，当前实现为兼容过渡；全量响应包装和 Schema 中间件尚未完成。
- Web API 默认不再启动 Worker，仅本地兼容场景可显式设置 `RUN_WORKERS_IN_API=true`。
- 已修正 PostgreSQL、IMAP 和爬取客户端默认跳过 TLS 验证的问题。

### 验证证据

- 后端 ESLint：活跃代码 0 error。
- 后端 Jest：7 个测试套件、31 个用例通过；当前 statements 覆盖率 38.75%，未达 70% 目标。
- 文档一致性检查：6 个 Model 通过。
- 前端 ESLint：0 error，仍有 63 个历史 warning。
- 前端生产构建：通过；主 JS 包约 793 kB，存在 chunk size warning。
- Docker Compose 配置校验、新增 JavaScript 语法检查和 `git diff --check`：通过。
- 依赖审计：后端 20 项（6 moderate、13 high、1 critical），前端 9 项（3 moderate、6 high）；存在无直接修复版本的依赖，CI 审计门禁尚未满足。

### 下一步计划

1. 获得授权后完成 Git 历史清理、全部凭据轮换和新克隆验收。
2. 在隔离 PostgreSQL 中执行全量 Migration、敏感数据迁移、回滚和备份恢复演练。
3. 完成邮件可持久化幂等/重试/补漏，以及爬虫刷新锁、冷却策略和持久化风控状态。
4. 补齐 API 契约层、后端集成测试、前端组件测试和端到端测试，将核心业务覆盖率提升到 70% 以上。
5. 评估并替换高风险无修复依赖，不使用破坏性 `audit fix --force`。

### 阻塞问题

- 未获得远程历史改写、凭据轮换、数据迁移或生产发布授权。
- 当前没有经确认的脱敏真实邮件/Apple HTML Fixture 和隔离 PostgreSQL 验收环境。

**最后更新**：2026-09-05

---

## 来源：docs/development/history/2026上半年.md
