...
const { encrypt, decrypt, blindIndex } = require('../utils/fieldEncryption');
  ...
  const Recipient = sequelize.define(
    'Recipient',
    {
      ...
      idCardNumber: {
        ...
        comment: '完整身份证号（AES-256-GCM 密文）',
        set(value) {
          this.setDataValue('idCardNumber', encrypt(value));
        },
        get() {
          return decrypt(this.getDataValue('idCardNumber'));
        },
      },
      idCardLast4: {
      ...
      password: {
        type: DataTypes.TEXT,
        allowNull: true,
        comment: '绑定的Apple账号密码（AES-256-GCM 密文）',
        set(value) {
    ...
    },
    {
      ...
      hooks: {
        beforeValidate: recipient => {
          const idCardNumber = recipient.idCardNumber;
          if (idCardNumber && !ID_CARD_PATTERN.test(idCardNumber)) {
            throw new Error('身份证号格式无效');
          }

          // 必须从明文 getter 派生，避免校验或截取 AES 密文。
          if (idCardNumber) {
            recipient.idCardLast4 = idCardNumber.slice(-4);
            recipient.idCardHash = blindIndex(idCardNumber);
          }
        },
      },
      comment: '收件人管理表',
