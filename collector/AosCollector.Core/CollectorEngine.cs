...
public sealed class CollectorEngine : IDisposable
{
  ...
  public CollectorStatus Status => new("运行中", connectionState, context?.ActiveSource ?? "unknown", lastScanAt, errorCode, store.Counts(), directories, files, Protocol.Now(), context?.ServerCounts, context?.ServerTime, "1.2.5", store.CodeCounts().Pending, store.CodeCounts().Errors);
      ...
      try {
        ...
        if (!authPaused && connectionState == "已连接" && DateTimeOffset.UtcNow - lastHeartbeat > TimeSpan.FromSeconds(15)) {
          try {
            ...
            context = await client.Heartbeat(new(Guid.NewGuid().ToString(), "1.2.5", Environment.OSVersion.VersionString, Protocol.Now(), lastScanAt, lastNewOrderAt, directories, store.Counts(), scanResults), token);
