...
const PREVIOUS_PROVIDERS = ['kdl_tunnel', 'kdl_private', 'fanproxy_tunnel'];
const CURRENT_PROVIDERS = [...PREVIOUS_PROVIDERS, 'yiyou_http'];
...
/** 为运行时代理切换状态加入亦优 HTTP Provider 枚举。 */
module.exports = {
  /** @param {import('sequelize').QueryInterface} queryInterface - Sequelize QueryInterface */
      ...
      await queryInterface.sequelize.query(
        `UPDATE order_refresh_system_states
         SET requested_proxy_provider = CASE
               WHEN requested_proxy_provider = 'yiyou_http' THEN NULL
               ELSE requested_proxy_provider
             END,
             active_proxy_provider = CASE
               WHEN active_proxy_provider = 'yiyou_http' THEN NULL
               ELSE active_proxy_provider
             END,
             proxy_switch_status = CASE
               WHEN requested_proxy_provider = 'yiyou_http'
                 OR active_proxy_provider = 'yiyou_http'
               THEN 'idle'
               ELSE proxy_switch_status
             END
         WHERE requested_proxy_provider = 'yiyou_http'
            OR active_proxy_provider = 'yiyou_http'`,
        { transaction }
      );
