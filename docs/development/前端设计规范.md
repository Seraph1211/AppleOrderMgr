...
| 用途       | Tailwind 类 | 大小 | 示例               |
| ---------- | ----------- | ---- | ------------------ |
| 页面主标题 | `text-2xl`  | 24px | 仪表板、订单管理   |
| 卡片标题   | `text-xl`   | 20px | 最近订单、订单信息 |
| 正文       | `text-sm`   | 14px | 表格内容           |
...
- 文字: `text-white`
- 圆角: `rounded-lg`
- 内边距: `px-3 py-1.5`，桌面文字 `text-sm`，最小高度 34px
...
```jsx
<input type="text" placeholder="搜索订单号..." className="input" />
```
...
<div className="relative">
  ...
  <input type="text" placeholder="搜索..." className="input pl-10" />
</div>
```
    ...
    <table className="w-full">
      <thead>
        <tr className="border-b border-gray-200 bg-gray-50">
          <th className="text-left py-3 px-4 text-sm font-medium text-gray-500">列标题</th>
        </tr>
      </thead>
      <tbody className="bg-white">
        <tr className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
          <td className="py-4 px-4 text-sm text-gray-600">单元格内容</td>
        </tr>
      </tbody>
    </table>
  </div>
...
取机人批量生成联系方式或地址时，首次确认用于确认生成意图；如果任一选中记录已有对应数据，必须再显示一次明确的覆盖警告，并说明受影响记录数。用户取消二次确认时不得调用 API，地址流程还需清理已暂存的省、市、区参数。
...
- 全局正文 14px、页面主标题 24px；桌面按钮和输入框最小高度 34px。手机输入保留 16px、主要操作至少 44px 点击区域。
- 桌面侧栏宽 208px、顶部栏高 52px、内容留白 16px；不使用整体 zoom 或裁切内容消除滚动条。
- 本人付款任务使用自适应固定布局表格，1366／1440／1920 CSS 像素、100% 比例下无横向溢出。日期和时间分行，输入框允许缩小，操作始终可见。
- 1600px 以下备注、付款人、最后爬数时间进入行内“更多”；1200px 以下 TAG 和下单时间也进入该区域。详情和主表共用编辑草稿及原权限，窄屏隐藏不改变数据授权。手机保留既有主要操作布局，不新增详情入口。
...
<aside className="hidden lg:block">{/* 侧边栏内容 */}</aside>;
...
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{/* 卡片 */}</div>;
```
...
<div className="overflow-x-auto">
  <table className="w-full">{/* 表格内容 */}</table>
</div>;
```
...
- [ ] 页面标题使用 `text-2xl font-bold text-gray-900`
...
页面实现参考 [Orders.jsx](../../frontend/src/pages/Orders.jsx)、[AppleIds.jsx](../../frontend/src/pages/AppleIds.jsx)、[Recipients.jsx](../../frontend/src/pages/Recipients.jsx)；仪表板统计参考 [Dashboard.jsx](../../frontend/src/pages/Dashboard.jsx)。现有代码可能仍有规范差距，参考入口不代表全部符合规范。

浅色、列表优先、标准按钮/输入/徽章、Lucide 图标和响应式检查仍为强制规则。新页面不得复制历史深色模板。Agent 初始化与角色模板统一见[Agent 提示词模板](Agent提示词模板.md)。
