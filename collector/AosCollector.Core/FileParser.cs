...
public static partial class FileParser
{
  ...
  public static List<ParsedFileLine> Parse(byte[] bytes, string encodingName, bool stable, bool successRecords = false)
  {
    ...
    try { text = encoding.GetString(bytes); }
      ...
      if (encodingName == "utf-8" && boundary > 0 && boundary < bytes.Length && error.Index >= boundary) {
        var completed = Parse(bytes[..boundary], encodingName, stable, successRecords);
    ...
    for (var index = 0; index < lines.Length; index++) {
      ...
      var complete = !successRecords && fields.Length >= 15 && date != null && fields.Where((_, i) => i is not 7 and not 8 and not 12).All(f => f.Length > 0);
      if (successRecords && stable) {
        try { PaymentCodeParser.Parse(line); complete = true; } catch (CollectorException) { complete = false; }
      }
