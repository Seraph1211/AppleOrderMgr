...
export default function EmailProcessing() {
  ...
  const openRecord = async id => {
    ...
    try {
      ...
      setDraft(normalizeDraft(response.data.manual_draft || response.data.parsed_data));
    } catch (requestError) {
  ...
  const addProduct = () => {
    setDraft(previous => ({
      ...previous,
      products: [...previous.products, { model: '', name: '', quantity: 1, image: null }],
    }));
  };
  ...
  const removeProduct = index => {
    setDraft(previous => ({
      ...previous,
      products: previous.products.filter((_product, productIndex) => productIndex !== index),
    }));
  };
  ...
  const handleIngest = async () => {
    ...
    setError('');
    try {
      const response = await ingestEmailRecord(detail.id, draft, detail.version);
  ...
  const handleResolve = async resolutionType => {
    ...
    if (resolutionType === 'existing_order') {
      orderNumber = window.prompt('请输入已有订单号：', draft.orderNumber || '');
    ...
    try {
      ...
      setDetail(null);
      setMessage(resolutionType === 'ignored' ? '邮件已标记忽略。' : '邮件已关联已有订单。');
      await Promise.all([loadRecords(), loadMetrics()]);
    } catch (requestError) {
  ...
  const handleBatchReparse = async () => {
    ...
    try {
      ...
      const succeeded = response.data.results.filter(result => result.success).length;
      setMessage(`批量重新解析完成：${succeeded}/${response.data.results.length} 成功。`);
      await Promise.all([loadRecords(), loadMetrics()]);
    } catch (requestError) {
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              ...
              <th className="px-4 py-3 text-left text-sm font-medium text-gray-500">推荐动作</th>
              ...
              records.map(record => (
                <tr key={record.id} className="border-b border-gray-200 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4 text-sm">
                    <p className="font-mono text-primary">{record.order_number || '-'}</p>
                    <p className="mt-1 text-red-600">
                      {record.error_code || record.error_message || '-'}
                    </p>
                    <p className="mt-1 text-xs text-gray-500">尝试 {record.retry_count} 次</p>
                  </td>
                  ...
                  <td className="px-4 py-4">
                    <span className={`badge ${BADGE_CLASSES[record.status] || 'badge-info'}`}>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/50">
          <div className="h-full w-full max-w-4xl overflow-y-auto bg-gray-50 shadow-xl">
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4">
              <div>
                <h2 className="text-xl font-semibold text-gray-900">邮件处理详情 #{detail.id}</h2>
            ...
            <div className="space-y-6 p-6">
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                ...
                <dl className="mt-4 grid gap-3 text-sm md:grid-cols-2">
                  <div>
                    ...
                    <dd className="break-all text-gray-900">{detail.email_from || '-'}</dd>
                  </div>
                  <div>
                    <dt className="text-gray-500">Subject</dt>
                    <dd className="text-gray-900">{detail.email_subject || '-'}</dd>
                  </div>
                  <div>
                    <dt className="text-gray-500">Message-ID</dt>
                    <dd className="break-all text-gray-900">{detail.message_id || '-'}</dd>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <dd className="text-red-600">{detail.error_message || '-'}</dd>
                  </div>
                </dl>
              </section>
              ...
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                ...
                <p className="mt-1 text-sm text-gray-500">标签必须来自订单信息行最后一个字段。</p>
              ...
              <section className="rounded-xl border border-gray-200 bg-white p-5">
                ...
                <div className="mt-4 overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200 bg-gray-50">
                        <th className="px-3 py-2 text-left text-sm text-gray-500">型号</th>
                        <th className="px-3 py-2 text-left text-sm text-gray-500">名称</th>
                        <th className="px-3 py-2 text-left text-sm text-gray-500">数量</th>
                        <th />
                      </tr>
                    </thead>
                    <tbody>
