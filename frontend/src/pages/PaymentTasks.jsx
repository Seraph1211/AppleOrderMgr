...
export default function PaymentTasks() {
  ...
  useEffect(() => {
    if (batchCopyAction?.message && !batchCopyAction.copying) {
      showToast(batchCopyAction.type, batchCopyAction.message);
    }
  ...
  const applyUpdatedTask = (taskId, updatedTask, draftPatch = {}) => {
    ...
    setDrafts(previous => ({
      ...
      [taskId]: {
        ...previous[taskId],
        status: updatedTask.processingStatus,
        ...draftPatch,
      },
    }));
  };
                      ...
                      <td colSpan={11 + (canSelectTasks ? 1 : 0)}>
                        <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">
                          <div className="min-w-0 space-y-1">
                            <p className="text-gray-600">处理备注</p>
                            {renderNotes(task)}
                          </div>
                          <div>
                            <p className="mb-1 text-gray-600">数据更新时间</p>
                            {renderTime(formatDateTime(task.lastCrawledAt))}
                          </div>
